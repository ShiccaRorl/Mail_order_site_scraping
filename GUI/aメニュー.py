...
layout = [
    [sg.Text('メニュー管理')],
    [sg.Button(button_text='通販データ取り込み', key="通販データ取り込み"), sg.Button(button_text='通販出品ジェネレーター', key="-通販出品ジェネレーター-")],  #
    # [sg.Button(button_text='通販',key="-通販-")],
    [sg.Button(button_text='ラクマ専用', key="ラクマ専用")],  # sg.Button(button_text='通販登録補助',key="-通販登録補助-"),
    ...
    [sg.Button(button_text='メール送信', key="メール送信"), sg.Button(button_text='作業ログ', key="作業ログ"), sg.Button(button_text='update', key="-update-")],
]
...
while True:
    ...
    elif event == "-通販出品ジェネレーター-":
        subprocess.Popen(["python", f"./a通販出品ジェネレーター.py", f"{id}"], shell=True)
